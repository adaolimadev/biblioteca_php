...
<body>
    ...
    <div class="container bg-light">
        <div class ="row"> 
            <div class = "col mt-5">
                <h1 class="text-center" >Lista de Empréstimos</h1><br>
                    ...
                    <tbody> 
                        <?php 
                            ...
                            if(!empty($_GET['search'])){
                                $data = $_GET['search'];
                                $sql = "SELECT * FROM vwEmprestimos WHERE cod LIKE '%$data%' or cliente LIKE '%$data%' or livro LIKE '%$data%' or obs LIKE '%$data%' ";
                            }else{
                                    $sql = "SELECT  * FROM  vwEmprestimos";
                            }
                            ...
                            while($linha = mysqli_fetch_assoc($result)){     
                                ...
                                <button class='btn btn-danger' onclick=\"
                                        if(confirm('Tem certeza que deseja o emprestimo de ID:".$linha['cod']." ? \\nEsta operação não poderá ser desfeita!')){
                                            location.href='action_livro.php?action=devolver&id_emprestimo=".$linha['cod']."&id_livro=".$linha['id_livro']."';
                                        }else{false;}\">
